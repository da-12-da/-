...
// Secrets: BAIDU_AK. Plain variable: ALLOWED_ORIGINS (comma-separated exact origins).
...
const optionalNumber=(value,max=Infinity)=>value!==null&&value!==undefined&&value!==''&&Number.isFinite(Number(value))&&Number(value)>=0&&Number(value)<=max?Number(value):undefined;
...
async function baidu(path,params,env,fetcher,signal){
 const url=new URL(path,'https://api.map.baidu.com');url.search=new URLSearchParams({...params,output:'json',ak:env.BAIDU_AK});
 let data;try{const response=await fetcher(url.href,{signal,redirect:'error'});if(!response.ok)throw Error('upstream');const body=await response.text();if(body.length>2000000)throw Error('size');data=JSON.parse(body);}catch{throw new ApiError(502,'BAIDU_UNAVAILABLE','地图服务暂时不可用，请稍后重试。');}
 if(Number(data.status)!==0){const quota=[4,302,401,402].includes(Number(data.status));throw new ApiError(quota?429:502,quota?'BAIDU_QUOTA':'BAIDU_REJECTED',quota?'地图查询额度暂时用尽，请稍后重试。':'地图服务未能完成查询，请检查服务权限或稍后重试。');}
 return data;
}
export function normalizeShops(results,center,radius){const seen=new Set();return results.flatMap(p=>{
 if(!p||!str(p.uid,100)||!str(p.name,200)||!coord(p.location)||seen.has(p.uid))return [];
 const meters=distance(center,p.location);if(meters>radius)return [];seen.add(p.uid);
 const detail=p.detail_info||{},rating=optionalNumber(detail.overall_rating,5),reviews=optionalNumber(detail.comment_num);
 const shop={id:p.uid,name:p.name,location:{lat:p.location.lat,lng:p.location.lng,coordType:'bd09ll'},distanceMeters:Math.round(meters)};
 if(str(p.address,300))shop.address=p.address;
 if(rating!==undefined)shop.rating=rating;
 if(Number.isSafeInteger(reviews))shop.reviewCount=reviews;
 // Place API has no documented positive-rate field. Never derive it from rating.
 return [shop];
 }).sort((a,b)=>(b.rating??-1)-(a.rating??-1)||a.distanceMeters-b.distanceMeters||a.id.localeCompare(b.id)).slice(0,3);}
export async function searchBaidu(query,env,fetcher=fetch){
 const signal=AbortSignal.timeout(12000);let center;
 if(query.location.kind==='region'){
  const data=await baidu('/geocoding/v3/',{address:query.location.label},env,fetcher,signal);center=data.result?.location;
 }else{
  const data=await baidu('/geoconv/v2/',{coords:`${query.location.lng},${query.location.lat}`,model:'2'},env,fetcher,signal);
  const point=data.result?.[0];center=point?{lng:point.x,lat:point.y}:null;
 }
 if(!coord(center))throw new ApiError(422,'LOCATION_NOT_FOUND','未找到这个位置，请输入更具体的城市和区域。');
 const data=await baidu('/place/v2/search',{query:query.category,tag:'美食',location:`${center.lat},${center.lng}`,coord_type:'3',radius:String(query.radiusMeters),radius_limit:'true',scope:'2',page_size:'20',page_num:'0',filter:'industry_type:cater|sort_name:distance|sort_rule:1'},env,fetcher,signal);
 if(!Array.isArray(data.results))throw new ApiError(502,'INVALID_DATA','地图返回数据异常，请稍后重试。');
 return {source:'百度地图 Place API',category:query.category,center:{...center,coordType:'bd09ll'},shops:normalizeShops(data.results,center,query.radiusMeters),ranking:'半径内候选按评分优先、距离次之；最多展示3家，不代表附近全部商家'};
}
...
export async function handle(request,env,fetcher=fetch){
 ...
 try{
  ...
  if(url.pathname==='/api/config')return json(200,{configured:Boolean(env.BAIDU_AK)});
  const query=parseSearch(url);
  if(!env.BAIDU_AK)throw new ApiError(503,'NOT_CONFIGURED','商家数据未接入。');
  if(await limited(request,env))throw new ApiError(429,'RATE_LIMITED','查询较频繁，请稍后再试。');
  return json(200,await searchBaidu(query,env,fetcher));
