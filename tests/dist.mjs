...
const root=new URL('../dist/',import.meta.url),files=['index.html','app.js','core.js','style.css','favicon.svg','success.wav','api/config','config.json'];
const server=http.createServer(async(req,res)=>{const pathname=req.url.startsWith('/-/')?req.url.slice(2):'/invalid';const name=pathname==='/'?'index.html':pathname.slice(1);if(!files.includes(name)){res.writeHead(404);res.end();return;}try{res.setHeader('Content-Type',name.endsWith('.js')?'text/javascript':name.endsWith('.css')?'text/css':name.endsWith('.wav')?'audio/wav':(name==='api/config'||name==='config.json')?'application/json':'text/html');res.end(await readFile(new URL(name,root)));}catch{res.writeHead(500);res.end();}});
...
try{const page=await browser.newPage({viewport:{width:390,height:844}}),errors=[];page.on('pageerror',e=>errors.push(e.message));
 assert.equal((await fetch(base+'/config.json')).status,200);await page.route('**/config.json',r=>r.fulfill({json:{configured:false}}));await page.goto(base+'/');assert.equal(await page.locator('.option').count(),0);assert.equal(await page.locator('#start').isDisabled(),true);
 ...
 await page.reload();assert.equal(await page.locator('.option').count(),2);const audio=await fetch(base+'/success.wav');assert.equal(audio.headers.get('content-type'),'audio/wav');assert.equal((await audio.arrayBuffer()).byteLength,352844);
